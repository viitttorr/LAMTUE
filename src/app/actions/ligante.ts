...
export async function finalizarSimulado(simuladoId: number, respostas: number[]) {
  const s = await exigirLigante();
  const sim = db().prepare("SELECT questoes, user_id, finalizado_em FROM simulados WHERE id = ?").get(simuladoId) as
...
export async function alternarModulo(tema: string) {
  const s = await exigirLigante();
  const existe = db().prepare("SELECT id FROM trilha_progresso WHERE user_id = ? AND tema = ?").get(s.id, tema);
  if (existe) db().prepare("DELETE FROM trilha_progresso WHERE user_id = ? AND tema = ?").run(s.id, tema);
  else db().prepare("INSERT INTO trilha_progresso (user_id, tema) VALUES (?, ?)").run(s.id, tema);
  revalidatePath("/ligante/trilha");
}

export async function registrarCaso(casoId: number, acertos: number, total: number) {
  const s = await exigirLigante();
  db().prepare("INSERT INTO casos_resultados (caso_id, user_id, acertos, total) VALUES (?, ?, ?, ?)").run(casoId, s.id, acertos, total);
}
