...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ehGabarito = await db().prepare("SELECT id FROM seletivo WHERE gabarito_arquivo_id = ?").get(arquivoId);
  ...
  if (!ehComprovante && !material && !ehExtensao && !foto && !ehEdital && !ehGabarito && !sessao) return new Response("Acesso negado", { status: 403 });
