...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ tipo: string }> }) {
  ...
  await registrarAcao(s.id, "relatorio_exportado", tipo);

  if (tipo === "frequencia") {
    const periodoAtual = await getConfig("periodo_atual", "2026/2");
    const ligantes = (await db().prepare("SELECT id, nome, matricula, email, semestre, turma FROM users WHERE role='ligante' ORDER BY nome").all()) as
    ...
    for (const l of ligantes) {
      const f = await frequenciaDe(l.id);
      const semestreCalc = calcularSemestre(l.turma, periodoAtual) ?? l.semestre;
      rows.push([l.nome, l.matricula, l.email, l.turma, semestreCalc, f.presentes, f.total, `${f.pct}%`, f.elegivel ? "sim" : "não"]);
    }
  ...
  if (tipo === "inscritos") {
    const rows = ((await db().prepare("SELECT nome, matricula, semestre, email, telefone, status, criado_em FROM inscricoes ORDER BY criado_em").all()) as
  ...
  if (tipo === "mensagens") {
    const rows = ((await db().prepare("SELECT canal, destinatario, assunto, evento, status, criado_em FROM mensagens ORDER BY id DESC").all()) as
  ...
  if (tipo === "financeiro") {
    ...
    const rows = ((await db().prepare("SELECT tipo, descricao, valor_centavos, data FROM financeiro ORDER BY data").all()) as
